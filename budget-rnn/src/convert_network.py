...
def convert_network(model_path: str, model_parameters: Dict[str, np.ndarray], precision: int, sensor_type: str, is_msp: bool):
    """
    Converts the neural network with weights at the given path to a C header file.

    Args:
        model_path: Path to the saved model weights
        model_parameters: The trainable model parameters (name -> value)
        precision: The fixed point precision to use during weight quantization
        sensor_type: The type of sensor (bluetooth or temperature). This variable
            determines the power profile to use.
        is_msp: Whether to create the header file for an MSP430 or a standard system.
    Returns:
        Nothing. The outputs are written to a file called "neural_network_parameters.h",
        overwriting an existing file if needed.
    """
    # Extract the model meta-data and hyper-parameters
    hypers = get_hyperparameters(model_path)
    metadata = get_metadata(model_path)

    model_name = hypers.model_params['model_type'].upper()
    model_name = model_name if model_name != 'SAMPLE_RNN' else 'BUDGET_RNN'
    model_type = SequenceModelType[model_name]
    ...
    # Make the offline power system to compute level estimates
...
if __name__ == '__main__':
    parser = ArgumentParser('Quantizes the neural network and converts the parameters into a C header file.')
