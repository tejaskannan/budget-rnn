...
SoftSkipUGRNNStateTuple = namedtuple('SoftSkipUGRNNStateTuple', ['state', 'state_update'])
...
# TODO: Add a Soft Skip UGRNN Cell
# TODO: Add a Random Skip UGRNN Cell
...
class SoftSkipUGRNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def get_initial_state(self, inputs: Optional[tf.Tensor], batch_size: Optional[int], dtype: Any) -> SkipUGRNNStateTuple:
        ...
        return SoftSkipUGRNNStateTuple(state=tf.tile(initial_state, multiples=(batch_size, 1)),
                                       state_update=tf.tile(initial_state_update, multiples=(batch_size, 1)))
    ...
    def __call__(self, inputs: tf.Tensor, state: SoftSkipUGRNNStateTuple, scope=None) -> Tuple[SoftSkipUGRNNOutputTuple, SoftSkipUGRNNStateTuple]:
        ...
        with tf.variable_scope(scope):
            ...
            # Compute the next state update probability
            next_state_update = tf.math.sigmoid(tf.matmul(next_state, self.W_state) + self.b_state)  # [B, 1]

            skip_state = SoftSkipUGRNNStateTuple(next_state, next_state_update)
            skip_output = SoftSkipUGRNNOutputTuple(next_state, state_update_gate)
