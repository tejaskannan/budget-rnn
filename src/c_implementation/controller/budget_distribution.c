...
ConfidenceBound get_budget(int32_t target, int32_t step, int32_t maxSteps, int32_t priorEnergy[NUM_OUTPUTS], BudgetDistribution *distribution, uint16_t precision) {
    ...
    int32_t stepFp = int_to_fp32(step, precision);
    int32_t energyRest = fp32_mul(expectedRest, int_to_fp32(stepDelta, precision), precision);
    int32_t expectedEnergy = fp32_div(fp32_sub(target, energyRest), stepFp, precision);
    ...
    int32_t estimatorVar = fp32_div(varianceRest, stepFp, precision);
